...
```move move filename="map_system.move" {6-13} copy showLineNumbers
...
```move move filename="map_system.move" {15-31} copy showLineNumbers
module monster_hunter::map_system;
use monster_hunter::schema::Schema;
use monster_hunter::position;
use monster_hunter::errors::already_registered_error;
...
useEffect(() => {
  const initialize = async () => {
    try {
      ...
      await subscribeToEvents(dubhe); // TODO: set the indexer api subscription
    } catch (error) {
