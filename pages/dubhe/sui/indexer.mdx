...
Query the indexed data using the Dubhe client:
...
// Query indexed storage data
const playerData = await dubhe.getStorageItem({
  name: "player",
  key1: "0x456...", // player address
});

console.log(playerData.value); // Parsed player data
```
...
dubhe convert-json --config-path dubhe.config.ts
...
For issues and feature requests related to the Rust indexer:

- **GitHub Issues**: [dubhe repository](https://github.com/0xobelisk/dubhe/issues)
- **Documentation**: [Dubhe Documentation](https://docs.dubhe.io)
