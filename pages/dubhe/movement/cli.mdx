...
The Dubhe CLI is a development toolkit for building and managing Dubhe projects on the Movement blockchain.

Core features:

1. `schemagen`: Generate Dubhe schemas automatically from your store schemas configuration
2. `publish`: Deploy your Dubhe projects to any Movement network (mainnet/testnet/devnet/localnet)
3. `localnode`: Manage local Movement node for development
4. `faucet`: Interface with Movement faucets to fund addresses on testnet/devnet/localnet
...
Deploy a Dubhe contract project to the Movement blockchain.
...
```bash
# to deploy Movement locally
dubhe publish --network localnet
# to deploy to Movement devnet
dubhe publish --network devnet
# to deploy to Movement testnet
dubhe publish --network testnet
# to deploy to Movement mainnet
dubhe publish --network mainnet
```

### `localnode`

The localnode command manages your local Movement node using the official `movement` binary.
