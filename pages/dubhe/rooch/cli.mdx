---
title: CLI - Dubhe
description: Learn about the Dubhe CLI for Rooch.
---
...
2. `publish`: Deploy your Dubhe projects to Rooch network
3. `localnode`: Manage local Rooch node for development
4. `faucet`: Interface with Rooch faucets to fund addresses
...
Deploy a Dubhe contract project to the Rooch blockchain.
...
```bash
# to deploy rooch locally
dubhe publish --network localnet
# to deploy to rooch devnet
dubhe publish --network devnet
# to deploy to rooch testnet
dubhe publish --network testnet
# to deploy to rooch mainnet
dubhe publish --network mainnet
```

### `localnode`

The localnode command manages your local Rooch node using the official `rooch` binary.
