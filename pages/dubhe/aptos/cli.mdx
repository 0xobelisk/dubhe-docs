...
3. `localnode`: Manage local Aptos node for development
4. `faucet`: Interface with Aptos faucets to fund addresses on testnet/devnet/localnet
...
Deploy a Dubhe contract project to the Aptos blockchain.
...
```bash
# to deploy Aptos locally
dubhe publish --network localnet
# to deploy to Aptos devnet
dubhe publish --network devnet
# to deploy to Aptos testnet
...
The localnode command manages your local Aptos node using the official `aptos` binary.
