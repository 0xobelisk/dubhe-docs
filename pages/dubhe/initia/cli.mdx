...
2. `publish`: Deploy your Dubhe projects to any Initia network (mainnet/testnet/devnet/localnet)
3. `localnode`: Manage local Initia node for development
4. `faucet`: Interface with Initia faucets to fund addresses on testnet/devnet/localnet
...
Deploy a Dubhe contract project to the Initia blockchain.
...
```bash
# to deploy Initia locally
dubhe publish --network localnet
# to deploy to Initia devnet
dubhe publish --network devnet
# to deploy to Initia testnet
dubhe publish --network testnet
# to deploy to Initia mainnet
dubhe publish --network mainnet
```

### `localnode`

The localnode command manages your local Initia node using the official `initia` binary.
